...
export function EmployeeProfileDrawer({
  ...
  const { year: scheduleYear, month: scheduleMonth } = getAppCurrentYearMonth()

  const monthlyScheduleQuery = useQuery({
    queryKey: ['monthly-schedule', scheduleYear, scheduleMonth],
    queryFn: () => monthlyScheduleService.getByYearMonth(scheduleYear, scheduleMonth),
